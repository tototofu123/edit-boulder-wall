...
export function generateTraverseRoute(ctx) {
    ...
    function scoreFootCandidate(candidate, h1, h2, isFinal, preferRecycledHands, routeHands) {
        ...
        if (!isFinal && belowHands < Math.max(0.18, 0.45 - densityFactor * 0.1))
            return Infinity;
        if (!isFinal && belowHands > Math.max(1.7, wingspan * 1.05))
            return Infinity;
        if (isFinal && belowHands < Math.max(0.12, 0.28 - densityFactor * 0.05))
            return Infinity;
