    ...
    def do_POST(self):
        if self.path == '/save':
            ...
            if data['csv']:
                ...
                fieldnames = ['cell_x', 'cell_y', 'category', 'num', 'type', 'difficulty', 'direction']
            ...
            if data:
                # Dynamically collect all keys from all objects to ensure no ValueErrors
                all_keys = set()
                for item in data:
                ...
                with open('docs/hold_annotations.csv', 'w', newline='') as f:
                    ...
                    writer.writeheader()
                    # Convert center object to string for CSV if it's a dict
                    csv_data = []
                    for item in data:
                        row = item.copy()
